...
void equal_range(void)
{
    ...
    {
        std::pair<std::map<int, std::string>::iterator, std::map<int, std::string>::iterator> p = m.equal_range(1);
       
	    for (std::map<int, std::string>::iterator &q = p.first; q != p.second; ++q) {
    ...
    {
        std::pair<std::map<int, std::string>::iterator, std::map<int, std::string>::iterator> pp = m.equal_range(-1);
        if (pp.first == m.begin()) {
    ...
    {
        std::pair<std::map<int, std::string>::iterator, std::map<int, std::string>::iterator> ppp = m.equal_range(3);
        if (ppp.first == m.end()) {
...
void key_comp(void)
{
    ...
    std::map<int, char, ModCmp>::key_compare comp_func = cont.key_comp();
 
    for (std::map<int, char, ModCmp>::iterator it = cont.begin(); it != cont.end();  it++) {
...
void rbegin_rend(void)
{
    ...
    for (std::map<int, std::string>::reverse_iterator it = coins.rbegin(); it != coins.rend(); ++it) {
...
void begin_end(void)
{
    ...
    for(std::map<Point *, double, PointCmp>::iterator iter = mag.begin(); iter != mag.end(); ++iter){
	...
	for (std::map<int, float>::iterator it = num_map.begin(); it != num_map.end(); ++it) {
		std::cout << it->first << ", " << it->second << '\n';
	}
	std::map<char,int> mymap;
...
void constructors(void)
{
  ...
  std::map<char,int,bool(*)(char,char)> fifth (fn_pt); // func
