...
/*   Updated: 2022/02/16 10:57:23 by arossign         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_input(char *str, t_flags flags)
{
	...
	if (flags.dot >= 0)
	{
		count = count + ft_width(flags.dot, ft_strlen(str), 0, flags.fd);
...
int	ft_string(char *str, t_flags flags)
{
	...
	if (flags.dot >= 0)
		count = count + ft_width(flags.width, flags.dot, 0, flags.fd);
	else
		count = count + ft_width(flags.width, ft_strlen(str), 0, flags.fd);
	if (flags.minus == 0)
		count = count + ft_input(str, flags);
	return (count);
}
