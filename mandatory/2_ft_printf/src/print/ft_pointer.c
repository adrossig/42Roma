...
/*   Updated: 2022/02/16 11:00:01 by arossign         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_input(char *pointer, t_flags flags)
{
	...
	if (flags.dot >= 0)
	{
		count = count + ft_width(flags.dot, ft_strlen(pointer), 1, flags.fd);
...
int	ft_pointer(unsigned long long num, t_flags flags)
{
	...
	if (flags.dot == 0 && !num)
	{
		if (flags.width >= 0 && flags.minus == 0)
			count += (ft_width(flags.width - 2, 0, 0, flags.fd) || 1)
				+ (ft_putsp("0x", 2, flags.fd) || 1);
		else
			count += (ft_putsp("0x", 2, flags.fd) || 1)
				+ (ft_width(flags.width - 2, 0, 0, flags.fd) || 1);
		return (count);
	}
	...
	count = count + ft_width(flags.width, ft_strlen(p) + 2, 0, flags.fd);
	if (flags.minus == 0)
		count = count + ft_input(p, flags);
	free(p);
	return (count);
