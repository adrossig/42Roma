...
{
	std::cout << std::endl
			  << "--> Information on ClapTrap And ScavTrap Before Testing..." << std::endl;
	bobbyClap->announce();
	yonScav->announce();
	std::cout << std::endl
			  << "--> Testing ScavTrap Member Function..." << std::endl;
	yonScav->attack(bobbyClap->getName());
	yonScav->takeDamage(100);
	yonScav->beRepaired(150);
	yonScav->guardGate();
	std::cout << std::endl
			  << "--> Information on ClapTrap And ScavTrap After Testing..." << std::endl;
	bobbyClap->announce();
	yonScav->announce();
}
...
int main(void)
{
	...
	std::cout << std::endl
			  << "--> Destroying ClapTrap And ScavTrap..." << std::endl;
	delete bobbyClap;
	delete yonScav;
	return (0);
}
